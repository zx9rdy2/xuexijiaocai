...
var sampleName = 'xinxijishu8',
	samplePath = 'samples/xinxijishu8/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.xinxijishu8 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.xinxijishu8 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.xinxijishu8').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.xinxijishu8').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.xinxijishu8').turn('next');

			},

			swipeRight: function() {
				
				$('.xinxijishu8').turn('previous');
