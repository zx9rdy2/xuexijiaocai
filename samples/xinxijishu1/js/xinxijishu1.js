...
var sampleName = 'xinxijishu1',
	samplePath = 'samples/xinxijishu1/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.xinxijishu1 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.xinxijishu1 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.xinxijishu1').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.xinxijishu1').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.xinxijishu1').turn('next');

			},

			swipeRight: function() {
				
				$('.xinxijishu1').turn('previous');
