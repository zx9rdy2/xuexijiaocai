...
var sampleName = 'xinxijishu6',
	samplePath = 'samples/xinxijishu6/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.xinxijishu6 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.xinxijishu6 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.xinxijishu6').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.xinxijishu6').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.xinxijishu6').turn('next');

			},

			swipeRight: function() {
				
				$('.xinxijishu6').turn('previous');
